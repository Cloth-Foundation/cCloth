...
function(compile_artifact output_digest target kind output)
    execute_process(
        COMMAND "${CLOTH_COMPILER}"
            ...
            --artifact-kind "${kind}"
            --output "${output}"
            --package arrays 0.1.0 "${CLOTH_SOURCE_ROOT}"
            --entry Main.co
        RESULT_VARIABLE result
        OUTPUT_VARIABLE receipt
        ERROR_VARIABLE diagnostics)
    if(NOT result EQUAL 0 OR NOT diagnostics STREQUAL "")
        message(FATAL_ERROR
...
foreach(target IN ITEMS x86_64 wasm32)
    set(interface "${CLOTH_WORK_DIRECTORY}/arrays-${target}-interface.cpa")
    compile_artifact(interface_digest "${target}" interface "${interface}")
endforeach()

set(object "${CLOTH_WORK_DIRECTORY}/arrays-x86_64-object.cpa")
compile_artifact(object_digest x86_64 object "${object}")

set(executable
    "${CLOTH_WORK_DIRECTORY}/runtime-sized-arrays${CLOTH_EXECUTABLE_SUFFIX}")
execute_process(
    COMMAND "${CLOTH_COMPILER}"
        --shuttle-protocol 2
        --operation link
        ...
        --root-package arrays
        --entry Main.co
        --artifact arrays 0.1.0 "${object_digest}" "${object}"
    RESULT_VARIABLE link_result
    OUTPUT_VARIABLE link_output
    ERROR_VARIABLE link_diagnostics)
if(NOT link_result EQUAL 0 OR NOT link_output STREQUAL "" OR
   NOT link_diagnostics STREQUAL "")
    message(FATAL_ERROR
        "runtime-sized array link failed:\n${link_output}${link_diagnostics}")
endif()
...
if(NOT run_result EQUAL 0 OR NOT run_diagnostics STREQUAL "")
    message(FATAL_ERROR
        "runtime-sized array executable failed:\n${run_diagnostics}")
endif()
