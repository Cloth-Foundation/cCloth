...
namespace cloth::debug {
    [[nodiscard]] inline const char *to_string(token::TokenKind kind) noexcept {
    ...
    [[nodiscard]] inline const char *to_string(token::Keyword keyword) noexcept {
        using KW = token::Keyword;
        switch (keyword) {
    ...
    [[nodiscard]] inline const char *to_string(token::Operator op) noexcept {
        ...
        switch (op) {
            ...
            case OP::ReturnArrow: return "ReturnArrow (:>)";
            default:
