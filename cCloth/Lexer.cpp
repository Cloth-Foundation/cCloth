...
namespace cloth {
	namespace lexer {
		...
		bool Lexer::eof() {
			return peek(0).token.kind == cloth::token::TokenKind::EndOfFile;
		}
		...
		cloth::token::Token Lexer::makeErrorToken(std::string_view message) {
			...
			cloth::token::Token t = makeToken(cloth::token::TokenKind::Error);
			return t;
		}

		cloth::meta_token::MetaToken Lexer::makeMetaToken(cloth::token::TokenKind kind) const {
			cloth::meta_token::MetaToken t;
			t.kind = kind;
			t.span = endTokenSpan();
			t.lexeme = tokenLexeme();
			return t;
